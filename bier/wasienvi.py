...
import numpy as np
from .utils import lazyprop, read_envi_header, parse_iso_date
from .geometry import rot_az_el_rev, constrain_az_el
...
class WasiEnvi(object):
    def __init__(self, fn, ureg=None):
        ...
        raw_shape = (lines, bands, samples)
        self.shape = (raw_shape[0], raw_shape[2], raw_shape[1])
        az = np.deg2rad(float(header['sun azimuth']))
        el = np.deg2rad(float(header['sun elevation']))
        self.sunPosition = (az, el)
        try:
            self.date = parse_iso_date(header['acquisition time'])
        except AttributeError:
            self.date = None

        self.refaz, self.refel, \
            self.azmin, self.elmin, \
            self.azstep, self.elstep \
            = map(float, header['map info'][1:7])

        self.azmin, self.elmin, \
            self.azstep, self.elstep \
            = map(np.deg2rad, (self.azmin, self.elmin, self.azstep, self.elstep))
        ...
        try:
            ...
            self.wvlns = None
        self.data = np.memmap(fn+'.raw', shape=raw_shape, dtype='float32').transpose(0, 2, 1)
    ...
    def shv(self):
        ...
        doRot = lambda c: constrain_az_el(*rot_az_el_rev(c, rotDelta, mirror=True))
